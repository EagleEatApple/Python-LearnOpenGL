...
class Blending_Sorted(CameraWindow):
    ...
    def init(self) -> None:
        ...
        # ------------------------------------
        self.shader = ProgramVF("shaders/4/3.2.blending.vs",
                                "shaders/4/3.2.blending.fs")
        ...
        self.transparentTexture = ImageTexture2D("textures/window.png")
    ...
    def render(self) -> None:
        ...
        for i in range(0, len(self.windows)):
            distance = glm.length(self.camera.position - self.windows[i])
            sorted_windows[distance] = self.windows[i]
        ...
        for distance in sorted(sorted_windows, reverse=True):
            model = glm.mat4(1.0)
            model = glm.translate(model, sorted_windows[distance])
            self.shader.setUniformMatrix4fv("model", model)
            glDrawArrays(GL_TRIANGLES, 0, 6)
 
if __name__ == "__main__":
    app = App()
    win = Blending_Sorted(title="Hello, Blending!")
    app.run(win)
