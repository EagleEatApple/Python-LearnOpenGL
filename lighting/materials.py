...
class Materials(CameraWindow):
    ...
    def init(self) -> None:
        ...
        self.lightingShader = ProgramVF("shaders/2/3.1.materials.vs", 
                                   "shaders/2/3.1.materials.fs")

        self.lightCubeShader = ProgramVF("shaders/2/3.1.light_cube.vs", 
                                   "shaders/2/3.1.light_cube.fs")
    ...
    def render(self) -> None:
        ...
        # light properties
        lightColor = glm.vec3()
        lightColor.x = sin(glfw.get_time() * 2.0)
        lightColor.y = sin(glfw.get_time()  * 0.7)
        lightColor.z = sin(glfw.get_time()  * 1.3)
        diffuseColor = lightColor * glm.vec3(0.5) # decrease the influence
        ambientColor = diffuseColor * glm.vec3(0.2) # low influence
        self.lightingShader.setUniform3fv("light.ambient", ambientColor)
        self.lightingShader.setUniform3fv("light.diffuse", diffuseColor)
        self.lightingShader.setUniform3f("light.specular", 1.0, 1.0, 1.0)

        # material properties
        self.lightingShader.setUniform3f("material.ambient", 1.0, 0.5, 0.31)
        self.lightingShader.setUniform3f("material.diffuse", 1.0, 0.5, 0.31)
        self.lightingShader.setUniform3f("material.specular", 0.5, 0.5, 0.5)
...
if __name__ == "__main__":
    app = App()
    win = Materials(title="Hello, Light!")
    app.run(win)