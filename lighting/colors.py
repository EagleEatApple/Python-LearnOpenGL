...
class Colors(CameraWindow):
    def init(self) -> None:
        ...
        self.lightingShader = ProgramVF("shaders/2/1.colors.vs", 
                                   "shaders/2/1.colors.fs")
        self.lightCubeShader = ProgramVF("shaders/2/1.light_cube.vs", 
                                   "shaders/2/1.light_cube.fs")
        ...
        self.cubeVAO.setVertexBuffer(self.VBO, 0, 0, 3 * sizeof(GLfloat))
        self.cubeVAO.setVertexAttribute(0, attribute_aPos)

        # second, configure the light's VAO (VBO stays the same; the vertices are the same for the light object which is also a 3D cube)
        self.lightCubeVAO = VertexArrayObject()
        self.lightCubeVAO.setVertexBuffer(self.VBO, 0, 0, 3 * sizeof(GLfloat))
