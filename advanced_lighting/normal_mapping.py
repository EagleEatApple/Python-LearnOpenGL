...
class NormalMapping(CameraWindow):
    ...
    def init(self) -> None:
        ...
        # ------------------------------------
        self.shader = ProgramVF("shaders/5/4.normal_mapping.vs",
                                "shaders/5/4.normal_mapping.fs")

        # load textures
        self.diffuseMap = ImageTexture2D("textures/brickwall.jpg")
        self.diffuseMap.setFiltering(GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR)
        self.diffuseMap.setWrapMode(GL_REPEAT, GL_REPEAT)
        self.normalMap = ImageTexture2D("textures/brickwall_normal.jpg")
...
if __name__ == "__main__":
    app = App()
    win = NormalMapping(title="Hello, Normal mapping!")
    app.run(win)